...
export default class Atlas {
  ...
  private readonly trails: P5.Graphics;

  constructor(p5: P5, images: Map<string, P5.Image>, trails: P5.Graphics) {
  ...
  private createTargets(
    p5: P5,
    images: Map<string, P5.Image | P5.Graphics>,
  ): void {
    this.targets.push(
      new Target(
        p5,
  ...
  private createObstacles(p5: P5, images: Map<string, P5.Image>): void {
  ...
  render(p5: P5, statistics: MissionStatistics, trails: P5.Vector[]): void {
    ...
    this.layouts.forEach((layout: Layout) => {
      layout.draw(p5, statistics);
    });
    this.targets.forEach((targets: Target) => {
      targets.draw();
    });
